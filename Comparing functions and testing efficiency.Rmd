---
title: "Comparing functions and testing efficiency"
...
```{r}
##This is an example and comparison
ex<-ahmd$Ex[,1]#crude death rate
dx<-ahmd$Dx[,1]#death count
x<-seq(0,length(ex),1)
...
existingtest<- LifeTable(x, Dx = dx, Ex = ex)

head(existingtest)
 
```
As shown, with slight decimal exceptions, the function I made is very similar to the existing function; and it comes with a plot as well! TOwards the end of the new datasets, the figures do start becoming more and more apart.

```{r}
system.time(LifeTable(x, Dx = dx, Ex = ex)) #existing function
system.time(myLT(x,ex,dx, initpop = 100000)) # my function
```

Unfortunately it appears my function is not as fast as the preesxisting one. 
